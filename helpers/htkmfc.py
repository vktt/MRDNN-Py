...
# Sphinx-III
##################################################
# Modified by: Vikrant Singh Tomar vikrant.tomar@mail.mcgill.ca
# McGill University
# Date: 21_Oct_2013
# Notes: When writing the HTK binary files, by default this script would write in the 
# native endian format of the machine. HTK by default reads big-endian, so you might need to 
# use the HTK NAUTRALREADORDER=TRUE configuration flag to read the created files correctly.
...
class HTKFeat_write(object):
    ...
    def __init__(self, filename=None,
                 veclen=13, paramKind = (MFCC | _O),
                 sampPeriod=100000, HTK_naturalWriteOrder=True):
        self.veclen = veclen
        ...
        self.dtype = 'f'
        self.filesize = 0
        self.islittleEndian = (unpack('=i', pack('>i', 42))[0] != 42)
        self.naturalWriteOrder = HTK_naturalWriteOrder
        if (filename != None):
            self.open(filename)
    ...
    def writeheader(self):
        self.fh.seek(0,0)
        if self.naturalWriteOrder and self.islittleEndian:
        	self.fh.write(pack("<IIHH", self.filesize, self.sampPeriod, self.sampSize,
                    	       self.paramKind))
        else:
        	self.fh.write(pack(">IIHH", self.filesize, self.sampPeriod, self.sampSize,
                    	       self.paramKind))
